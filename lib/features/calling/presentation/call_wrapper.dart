...
/// Wraps the entire app and shows a call overlay when there's an active call
class CallWrapper extends ConsumerWidget {
  final Widget child;

  const CallWrapper({super.key, required this.child});
...
class _CallOverlayScreen extends ConsumerStatefulWidget {
  const _CallOverlayScreen();

  @override
  ConsumerState<_CallOverlayScreen> createState() => _CallOverlayScreenState();
}

class _CallOverlayScreenState extends ConsumerState<_CallOverlayScreen>
      ...
      child: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              flex: 3,
              child: _buildCallInfo(context, call, isConnecting),
            ),

            // Middle section with call actions
            Expanded(
              flex: 2,
              child: _buildCallActions(context, call),
            ),

            // Bottom section with end call button
            Expanded(
              flex: 2,
              child: _buildEndCallSection(context, call),
            ),
          ],
        ),
      ),
  ...
  Widget _buildCallInfo(BuildContext context, ActiveCall call, bool isConnecting) {
    return Column(
  ...
  Widget _buildCallActions(BuildContext context, ActiveCall call) {
  ...
  Widget _buildEndCallSection(BuildContext context, ActiveCall call) {
    final callNotifier = ref.read(callProvider.notifier);
    final isConnected = call.status == CallStatus.connected ||
        call.status == CallStatus.onHold;
